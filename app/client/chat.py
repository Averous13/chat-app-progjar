...
class Chat:
    ...
    def create_group(self, tokenid, groupname):
        ...
        return {"status": "OK", "message": "Grup {} berhasil dibuat".format(groupname)}
    ...
    def send_group_message(self, sessionid, sender, usernameto_list, message):
        if sessionid not in self.sessions:
            return {"status": "ERROR", "message": "Informasi tidak ditemukan"}
        if not all(username in self.users for username in usernameto_list):
            return {"status": "ERROR", "message": "Satu atau lebih username tidak ditemukan"}
        for usernameto in usernameto_list:
            inbox = self.users[usernameto]["incoming"].get("inbox", [])
            inbox.append({"from": sender, "message": message})
            self.users[usernameto]["incoming"]["inbox"] = inbox
    ...
    def send_group_file(self, sessionid, sender, usernameto_list, filename, filedata):
        if sessionid not in self.sessions:
            return {"status": "ERROR", "message": "Informasi tidak ditemukan"}
        if not all(username in self.users for username in usernameto_list):
            return {"status": "ERROR", "message": "Satu atau lebih username tidak ditemukan"}
        for usernameto in usernameto_list:
            inbox = self.users[usernameto]["incoming"].get("inbox", [])
            inbox.append({"from": sender, "filename": filename, "filedata": filedata})
            self.users[usernameto]["incoming"]["inbox"] = inbox
